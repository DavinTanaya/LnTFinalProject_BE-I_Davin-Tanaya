...
<body>
    ...
    <form method="POST" action="{{route('update.cat', ['id' => $item->id])}}" class="content">
        @csrf
        <h1>Add Category to Book</h1>
                    ...
                    @foreach($categories as $c)
                        <li>
                            <label class="dropdown-item">
                                <input type="checkbox" name="cat_ids[]" value="{{$c->id}}" onclick="toggleCategory(this)">
                                {{$c->categoryName}}
                            </label>
                        </li>
                    @endforeach
